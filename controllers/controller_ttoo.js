			...
			if (results.length > 0) {
				...
				db.query(
					...
					(error, results) => {
						...
						res.redirect('/ajustes/conf_ttoo')
					}
				)
			}
		})
...
exports.getOperaCodes = (req, res) => {
	try {
		db.query('SELECT * FROM opera_codes WHERE nombre LIKE "%agencia%"', (error, results) => {
			if (results) {
				res.render('ajustes/new_ttoo', { logged: req.user, alert: false, results: results, error: false, message: req.flash('message') })
			} else {
				throw error
			}
		})
...
exports.getTtoos = (req, res) => {
	try {
		db.query('SELECT * FROM ttooRates order by nombre asc', (error, results) => {
			if (results) {
				res.render('ajustes/conf_ttoo', { logged: req.user, alert: false, results: results, error: false, message: req.flash('message') })
			} else {
				throw error
			}
		})
...
exports.getTtoo = (req, res) => {
	try {
		...
		db.query('SELECT * FROM ttooRates WHERE id=?', [id], (error, results) => {
			if (results) {
				res.render('ajustes/edit_ttoo', { logged: req.user, ttoo: results[0], alert: false })
			} else {
				throw error
			}
		})
