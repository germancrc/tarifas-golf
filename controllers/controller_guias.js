	...
	try {
		...
		db.query(
			...
			(error, results) => {
				...
				req.flash('message', 'La guía ' + aplicacion + ' fué agregada con éxito')
				res.redirect('/ajustes/conf_guias')
			}
		)
	} catch (error) {
		console.log(error)
...
exports.getGuide = (req, res) => {
	try {
		...
		db.query(
			...
			(error, results) => {
				if (results) {
					res.render('ajustes/edit_guia', { logged: req.user, guia: results[0], alert: false })
				} else {
					throw error
				}
			}
