	...
	try {
		...
		db.query('SELECT codigo FROM opera_codes where codigo = ?', [codigo], (error, results) => {
			if (results.length > 0) {
				...
				res.redirect('/ajustes/new_codigo_opera')
			} else {
				db.query('INSERT INTO opera_codes SET ?', { codigo: codigo, nombre: nombre, uso: uso, descripcion: descripcion }, (error, results) => {
					if (results) {
						...
						res.redirect('/ajustes/conf_codigos_opera')
					}
				})
			}
		})
...
exports.getOperaCodes = (req, res) => {
	try {
		res.render('ajustes/new_codigo_opera', { logged: req.user, alert: false, error: false, message: req.flash('message') })
	} catch (error) {
		console.log(error)
	}
}
...
exports.getCodes = (req, res) => {
	try {
		db.query('SELECT * FROM opera_codes order by uso, codigo asc', (error, results) => {
			if (results) {
				res.render('ajustes/conf_codigos_opera', {
					logged: req.user,
					alert: false,
					results: results,
					error: false,
...
exports.getCode = (req, res) => {
	try {
		...
		db.query('SELECT * FROM opera_codes WHERE id=?', [id], (error, results) => {
			if (results) {
				res.render('ajustes/edit_codigo_opera', { logged: req.user, code: results[0], alert: false })
			} else {
				throw error
			}
		})
